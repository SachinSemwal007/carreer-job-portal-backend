...
app.post("/api/createpost", async (req, res) => { 
  const { jobTitle, skillsRequired, experienceRequired, educationalBackground, location, salary, jobDescription } = req.body; 
    ...
    try { 
      ...
      if (!applicantId || !firstName || !lastName || !email || !contact) { 
        return res.status(400).json({ 
          message: "All required fields (applicantId, firstName, lastName, email, contact) must be provided.", 
        }); 
      } 
...
app.put( 
  ...
  async (req, res) => { 
    ...
    try { 
      ...
      // Update Applicant 
      await Applicant.updateOne({ "appliedPositions.applicationId": applicationId }, updateApplicant, { arrayFilters: [{ "elem.applicationId": applicationId }], session: session }); 
 
      // Update JobPost 
      await Post.updateOne({ "applicants.applicationId": applicationId }, updateJobPost, { arrayFilters: [{ "elem.applicationId": applicationId }], session: session }); 
 
      // Commit transaction 
      await session.commitTransaction(); 
      session.endSession(); 
 
      res.status(200).json({ message: "Applicant and Job post updated successfully" }); 
    } catch (error) { 
      await session.abortTransaction(); 
      session.endSession(); 
...
app.delete("/api/posts/:postId/applications/:applicationId", async (req, res) => { 
  ...
  try { 
    ...
    const applicationIndex = jobPost.applicants.findIndex((app) => app._id.toString() === applicationId); 
